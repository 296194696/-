/** layuiAdmin.std-v1.1.0 LPPL License By http://www.layui.com/admin/ */
 ;layui.define(["table","form"],function(e){var t=layui.$,i=layui.table;var form=layui.form;i.render({elem:"#LAY-user-manage",url:"user/info/readall",cols:[[{type:"checkbox",fixed:"left"},{field:"indocno",width:100,title:"ID",sort:!0},{field:"username",title:"用户名",minWidth:100},{field:"role",title:"角色"},{field:"rolename",title:"角色名"},{field:"sname",title:"昵称"},{field:"identity",title:"身份证"},{title:"操作",width:150,align:"center",fixed:"right",toolbar:"#table-useradmin-webuser"}]],page:!0,limit:30,height:"full-220",text:"对不起，加载出现异常！"}),i.on("tool(LAY-user-manage)",function(e){
     tbldata=e.data;
     if("del"===e.event)layer.prompt({formType:1,title:"敏感操作，请验证口令"},function(t,i){layer.close(i),layer.confirm("真的删除行么",function(t){
         e.del(),
             $.ajax({
                 ...
                 dataType:'json',
                 data:{
                     delLists : parseInt(tbldata.indocno)
                 },
             ...
             layer.close(t)})});else if("edit"===e.event){t(e.tr);layer.open({type:2,title:"编辑用户",content:"/userform",maxmin:!0,area:["500px","450px"],btn:["确定","取消"],yes:function(e,t){var l=window["layui-layer-iframe"+e],r="LAY-user-front-submit",n=t.find("iframe").contents().find("#"+r);l.layui.form.on("submit("+r+")",function(t){
     var entity=t.field;
        $.ajax({
            ...
            dataType:'json',
            data:{
                indocno : tbldata.indocno,
                iroleid: tbldata.iroleid,
